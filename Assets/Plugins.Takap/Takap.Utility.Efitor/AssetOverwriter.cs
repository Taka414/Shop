...
namespace Takap.Utility
{
    ...
    public class AssetOverwriter : AssetPostprocessor
	{
        ...
        private const string SourceExistFormat = "「{0}」と「{1}」のファイル内容が完全一致しているため、置き換えツールの動作を停止します。\n本当にインポートしますか？";
        private const string MessageFormat = "「{0}」という名前のアセットが既に存在します。アセットを置き換えますか？";
        ...
        private static void OnPostprocessAllAssets(string[] importedAssets,	string[] deletedAssets,	string[] movedAssets, string[] movedFromPath)
		{
			...
			foreach (ExistAsset exist in existAssets)
			{
