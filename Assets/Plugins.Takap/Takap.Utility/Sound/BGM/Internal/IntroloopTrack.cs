...
namespace Takap.Utility.Sound
{
    ...
    internal class IntroloopTrack : MonoBehaviour
    {
        ...
        ///         on calling <see cref="BgmPlayer.Play(IntroloopAudio,float,float)"/>
        ///     </para>
        ///     <para>
        ...
        ///     </para>
        /// </summary>
        const float _smallPrepareTime = 0.02f;

        // デバッグメッセージを使用するかどうかのフラグ
        // true: 使用する / false: 使用しない
        static readonly bool _useDebugMessage = false;

        //
        // Fields
        // - - - - - - - - - - - - - - - - - - - -

        private double _dspPlusHalfAudio;

        private float _fadeVolume;
        private double _introFinishDspTime;
        //internal IntroloopSettings introloopSettings;

        private bool _isPausing;
        private bool _isPausingOnIntro;
        ...
        private double _pauseDspTimestamp;

        /// <summary>
        ///     When resume, this variable will add up how long we have paused the audio.
        ///     Used in determining total playtime.
        /// </summary>
        private double _pauseDspTotal;

        private double _playDspTimestamp;
        private int _playingSourceBeforePause;
        private double _rememberHowFarFromNextTransitionTime;
        private double[] _sourceWillEndTime;
        private double[] _sourceWillPlayTime;
        private float _startPlayingUnscaledClipTime;
        ...
        private AudioSource[] _twoSources;

        //
        // Props
        // - - - - - - - - - - - - - - - - - - - -

        internal BgmAudioClip Music { get; private set; }

        internal BgmAudioClip MusicAboutToPlay { get; private set; }
        ...
        internal float PlayedTimeSecondsUnscaled
        {
            get
            {
                if (!IsPlaying && !_isPausing)
                {
                    return 0;
                }

                double currentDspPlayhead;
                if (_isPausing || !IsPlaying)
                {
                    currentDspPlayhead = _pauseDspTimestamp;
                }
                else
                {
                ...
                return (float)(currentDspPlayhead - _playDspTimestamp - _pauseDspTotal);
            }
        }
        ...
        internal float PlayheadPositionSeconds
        {
            get
            {
                ...
                if (Music.NonLooping)
                {
                    return Mathf.Min(_startPlayingUnscaledClipTime + playedTime * Music.Pitch, Music.UnscaledClipLength);
                }

                if (Music.LoopWholeAudio)
                {
                    return (_startPlayingUnscaledClipTime + playedTime * Music.Pitch) % Music.UnscaledClipLength;
                }

                var playedUnscaled = _startPlayingUnscaledClipTime + playedTime * Music.Pitch;
        ...
        public string[] DebugInformation
        {
            get
            {
                return new[]
                {
                    ...
                    "Source 1 Will Play :" + _sourceWillPlayTime[0].ToString(".00"),
                    "Source 1 Will End :" + _sourceWillEndTime[0].ToString(".00"),
                    "Source 2 Will Play :" + _sourceWillPlayTime[1].ToString(".00"),
                    "Source 2 Will End :" + _sourceWillEndTime[1].ToString(".00"),
                    "Source 1 : " + (_twoSources[0].isPlaying ? "PLAYING/SCHEDULED" : "STOPPED"),
                    "Source 2 : " + (_twoSources[1].isPlaying ? "PLAYING/SCHEDULED" : "STOPPED"),
                    "Source 1 Time : " + _twoSources[0].time.ToString(".00"),
                    "Source 2 Time : " + _twoSources[1].time.ToString(".00"),
                    "Next transition time : " + _nextTransitionTime.ToString(".00"),
                    "Dsp plus half audio : " + _dspPlusHalfAudio.ToString(".00"),
                    "Is pausing : " + _isPausing,
                };
            }
        }
        ...
        internal IEnumerable<AudioSource> AllAudioSources
        {
            get
            {
                if (_twoSources == null)
                {
                    throw new System.InvalidOperationException("Introloop is not yet initialized. Please avoid accessing internal AudioSource on Awake.");
                }
                return _twoSources;
            }
        }
        ...
        private void Awake()
        {
            _twoSources = new AudioSource[2];
            _sourceWillPlayTime = new double[2];
            _sourceWillEndTime = new double[2];
        ...
        internal void Unload()
        {
            ...
            _twoSources[0].clip = null;
            _twoSources[1].clip = null;
            Music = null;
        ...
        internal void SchedulingUpdate()
        {
            if (IsPlaying)
            {
                ...
                {
                    _dspPlusHalfAudio = AudioSettings.dspTime + Music.LoopingLength / 2f;
                    if (_dspPlusHalfAudio > _nextTransitionTime)
                    {
        ...
        private void ScheduleNextLoop()
        {
            ...
            SetScheduledEndTime((_nextSourceToPlay + 1) % 2, _nextTransitionTime);
            PlayScheduled(_nextSourceToPlay, _nextTransitionTime);
            _twoSources[_nextSourceToPlay].time = Music.UnscaledIntroLength;

            _nextTransitionTime += (Music.LoopWholeAudio ? Music.Length : Music.LoopingLength);

            _nextSourceToPlay = (_nextSourceToPlay + 1) % 2;
        ...
        internal void Stop()
        {
            _twoSources[0].Stop();
            _twoSources[1].Stop();
            _pauseDspTimestamp = AudioSettings.dspTime;
        ...
        internal bool IsPausable()
        {
            ...
            if (!IsPlaying || !_twoSources[0].isPlaying && !_twoSources[1].isPlaying)
            {
                return false;
            }
        ...
        internal void Pause()
        {
            ...
            if (_twoSources[0].isPlaying && _twoSources[1].isPlaying)
            {
                // Hard case, which one is not actually playing but scheduled?
                // (Scheduled audio reports isPlaying as true)

                if (AudioSettings.dspTime < _sourceWillPlayTime[0])
                {
                    _playingSourceBeforePause = 1;
                }
                else
                {
                    _playingSourceBeforePause = 0;
                }
            }
            else
            {
                // Easy case
                if (_twoSources[0].isPlaying)
                {
                    _playingSourceBeforePause = 0;
                }
                else
            ...
            _rememberHowFarFromNextTransitionTime = _nextTransitionTime - pausingDspTime;
            _pauseDspTimestamp = pausingDspTime;

            if (!Music.NonLooping && !Music.LoopWholeAudio) //If contain intro
            {
                if (pausingDspTime >= _introFinishDspTime)
                {
                    _isPausingOnIntro = false;
                }
                else
                {
                    _isPausingOnIntro = true;
                }
            }
        ...
        internal bool Resume()
        {
            ...
            var sourceToContinuePlaying = _playingSourceBeforePause;

            // Rescheduling!
            var absoluteTimeNow = AudioSettings.dspTime;

            _pauseDspTotal += absoluteTimeNow - _pauseDspTimestamp;

            float remainingTime;
            if (!Music.NonLooping && !Music.LoopWholeAudio) //If contain intro
            {
                if (_isPausingOnIntro)
                {
                    remainingTime = Music.IntroLength - _twoSources[sourceToContinuePlaying].time / Music.Pitch;
                }
                else
                {
                    remainingTime = Music.IntroLength + Music.LoopingLength -
                                    _twoSources[sourceToContinuePlaying].time / Music.Pitch;
                }
            }
            else
            {
                remainingTime = Music.Length - _twoSources[sourceToContinuePlaying].time / Music.Pitch;
            }
            ...
            if (_isPausingOnIntro)
            {
                ...
                _introFinishDspTime = absoluteTimeNow + remainingTime;
            }

            _nextTransitionTime = AudioSettings.dspTime + _rememberHowFarFromNextTransitionTime;
        ...
        internal void Play(BgmAudioClip music, bool isFade, float startTime)
        {
            _pauseDspTimestamp = 0;
            _pauseDspTotal = 0;

            _twoSources[0].pitch = music.Pitch;
            _twoSources[1].pitch = music.Pitch;
        ...
        private IEnumerator LoadAndPlayRoutine(BgmAudioClip music, float startTime)
        {
            var musicName = music.AudioClip.name;
        ...
        private void SetupPlayingSchedule(BgmAudioClip music, float startTime)
        {
            ...
            _twoSources[0].clip = music.AudioClip;
            _twoSources[1].clip = music.AudioClip;
            ...
            var dspTimeNow = AudioSettings.dspTime + _smallPrepareTime;

            if (music.NonLooping)
            {
                if (startTime < music.UnscaledClipLength)
                {
                    _twoSources[0].time = startTime;
                    _twoSources[1].time = 0;

                    // Note : you cannot just pull the value back from `twoSources[0].time`. The setter is not instantaneous!
                    _startPlayingUnscaledClipTime = startTime;
                ...
                else
                {
                    ...
                    _twoSources[0].time = 0;
                    _twoSources[1].time = 0;
                    _startPlayingUnscaledClipTime = music.UnscaledClipLength;
                }
            ...
            else if (music.LoopWholeAudio)
            {
                ...
                _twoSources[0].time = loopedStartTime;
                ...
                var dspEndMusicTime = dspTimeNow + music.Length - startTime / music.Pitch;
                PlayScheduled(0, dspTimeNow);
                SetScheduledEndTime(0, dspEndMusicTime);
                _introFinishDspTime = dspEndMusicTime;

                PlayScheduled(1, dspEndMusicTime);
                _nextTransitionTime = dspEndMusicTime + music.Length;
                _startPlayingUnscaledClipTime = loopedStartTime;
            ...
            else
            {
                ...
                if (beforeIntro)
                {
                    ...
                    _twoSources[1].time = music.UnscaledIntroLength;

                    PlayScheduled(0, dspTimeNow);
                    SetScheduledEndTime(0, dspIntroSeamTime);
                    _introFinishDspTime = dspIntroSeamTime;

                    PlayScheduled(1, dspIntroSeamTime);
                    _nextTransitionTime = dspIntroSeamTime + music.LoopingLength;
                    _startPlayingUnscaledClipTime = startTime;
                }
                else
                {
                    ...
                    _twoSources[0].time = music.UnscaledIntroLength + unscaledTimeIntoTheLoop;
                    // If start time is over intro, the 2nd source must be waiting at the end instead.
                    _twoSources[1].time = music.UnscaledIntroLength + music.UnscaledLoopingLength;
                    ...
                    _nextTransitionTime = dspLoopEndTime;
                    ...
                    _introFinishDspTime = dspTimeNow - scaledTimeIntoTheLoop;

                    _startPlayingUnscaledClipTime = music.UnscaledIntroLength + unscaledTimeIntoTheLoop;
                    ;
                }
        ...
        internal void ApplyVolume()
        {
            if (Music != null)
            {
                _twoSources[0].volume = FadeVolume * Music.Volume;
                _twoSources[1].volume = FadeVolume * Music.Volume;
            }
        }
        ...
        private void PlayScheduled(int sourceNumber, double absoluteTime)
        {
            ...
            _twoSources[sourceNumber].PlayScheduled(absoluteTime);
            _sourceWillPlayTime[sourceNumber] = absoluteTime;
        }

        private void SetScheduledEndTime(int sourceNumber, double absoluteTime)
        {
            _twoSources[sourceNumber].SetScheduledEndTime(absoluteTime);
            _sourceWillEndTime[sourceNumber] = absoluteTime;
        }

        private void SetScheduledStartTime(int sourceNumber, double absoluteTime)
        {
            _twoSources[sourceNumber].SetScheduledStartTime(absoluteTime);
            _sourceWillPlayTime[sourceNumber] = absoluteTime;
        }
