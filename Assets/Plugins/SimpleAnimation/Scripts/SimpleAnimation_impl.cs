...
#pragma warning disable IDE0009, IDE0044, IDE1006, IDE0017
...
public partial class SimpleAnimation: MonoBehaviour, IAnimationClipSource
{
    ...
    [SerializeField]
    //[ListDrawerSettings(DraggableItems = true, Expanded = false, ShowIndexLabels = true, ShowPaging = false, ShowItemCount = false)]
    ...
    private void Initialize()
    {
        ...
        if (m_States != null)
        {
            foreach (EditorState state in m_States)
            {
                if (state.clip)
                {
    ...
    {
        IEnumerable<State> playableStates = GetStates();
        var list = new List<EditorState>();
        foreach (State state in playableStates)
        {
